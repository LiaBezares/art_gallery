...
struct State
{
...

    {
        ...
        if (fullscreen)
            window.create(sf::VideoMode::getDesktopMode(), sf::String(title), sf::State::Fullscreen, settings);
        else
        ...
        // El raton arranca libre (visible, sin capturar) para poder mover/redimensionar la ventana.
        // Se activa con la tecla Tab cuando el usuario quiera controlar la camara.
    ...
    void handleEvents()
    {
        while (const std::optional event = window.pollEvent())
        {
            ...
            {
                // Liberar el raton al perder el foco (alt-tab, clic fuera, etc.)
                isMouseCaptured = false;
...
int main()
{
    State state(800, 600, "3D Virtual Art Gallery - Tappa 12");
